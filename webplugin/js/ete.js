var ete_webplugin_URL = "http://coretracker/wsgi/wsgi.py";
...
function coretracker(treeid, recipient, seq, extra_params) {
	var params = {
		"treeid": treeid,
		"seqtype": seq
	};
	...
	$(recipient).html(loading_img);
	//$(recipient).fadeTo(500, 0.2);
	$(recipient).load(ete_webplugin_URL + '/track', params, function() {
		$(recipient).fadeTo(100, 1);
	});
}
...
function show_context_menu(treeid, nodeid, actions, textface) {
	...
	$('#popup').load(ete_webplugin_URL + '/get_menu', {
		"treeid": treeid,
...
function run_action(treeid, nodeid, aindex, search_term) {
	...
	$(recipient).load(ete_webplugin_URL + '/action', {
		"treeid": treeid,
		"nid": nodeid,
		"aindex": aindex,
