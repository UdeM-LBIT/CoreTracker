...
def get_domain_in_gene(gene, outdir, path_to_db, addargs=""):
    ...
    build_cmd = "hmmscan -o %s --domtblout %s %s %s %s"%(outmp, outfile, path_to_db, seqfile, addargs)
    print(build_cmd)
    utils.executeCMD(build_cmd, 'hmmscan')
    cut_cmd = "grep -v '#' %s | sed 's/  */\t/g' |cut -f 1,2,4,7,8,18,19,20,21,23- > %s"%(outfile, outfile+".tmp")
    ...
    with open(outf) as OUT:
        ...
        for line in OUT:
            ...
            if line:
                ...
                spec_do.add_domain(l[0], l[1], int(l[7])-1, int(l[8])-1, " ".join(l[9:]), l[3], l[4])
            ...
            for pos in gpos[spec]:
                ...
                for (name, s_e) in gdomain[spec].position.items():
                    ...
                    if start <= pos and pos<=end:
                        ...
                        break
                if doname:
                    dchecked.append((pos, doname, gdomain[spec].pfam[doname], gdomain[spec].description[doname], str(start), str(end)))

                else:
                    dchecked.append((pos, "NID" , "NID", "NID", '-', '-'))
        else:
            dchecked = [(pos, "N/A", "N/A","N/A", '-', '-') for pos in gpos[spec]]
        rdata[spec] = dchecked
    return rdata
...
def export_data_as(data, outfile, format='json'):
    if format=='csv':
        with open(outfile, 'w') as OUT:
            OUT.write("\t".join(['Species', 'Gene', 'Codon', 'AA', 'Pos', 'Domain', 'Pfam', 'start', 'end', 'description'])+"\n")
            for (gene, rea) in data.items():
                for r, specmap in rea.items():
                    codon, aa = r.split(':')
                    for spec, posmap in specmap.items():
                        for pos in posmap:
                            OUT.write("\t".join([spec, gene, codon, aa, str(pos[0]), pos[1], pos[2], str(pos[4]), str(pos[5]), pos[3]])+"\n")
    elif format=='json':
        with open(outfile, 'w') as OUT:
...
if __name__ == '__main__':
    curdir = os.getcwd()
    fchoices = ['json', 'csv']
    parser = argparse.ArgumentParser(description='Find information about position')
    parser.add_argument('--corefile', '-c',  required=True, dest="corefile", help="Protein corefile")
    parser.add_argument('--posfile', '-p',  required=True, dest="posfile", help="Position file in json format")
    parser.add_argument('--outdir', dest='outdir', default=curdir,  help="Output directory")
    parser.add_argument('--outfile', '--out', '-o', dest='outfile', default='outfile',  help="Output file")
    parser.add_argument('--pfamhmm', '--hmm',  required=True, dest="hmmdb", help="PFam hmm. ou should run hmmpress before")
    parser.add_argument('--hmmscan_args', '--extra_args', default="", dest="extrarg", help="Position in alignment, separated by comma or a file")
    parser.add_argument('--align', '-a', action='store_true', dest="aligned", help="save data as alignment intead of sequences")
    parser.add_argument('--format', '--fmt', choices=fchoices, dest="format", help="Format to save the data in 9json or csv")

    args = parser.parse_args()
    dt = domain_checker(args.posfile, args.corefile, args.outdir, args.hmmdb, args.extrarg, aligned=args.aligned)
