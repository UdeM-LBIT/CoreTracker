...
class RunningInstance():
    ...
    def get_model(self, etiquette):
        clf = Classifier.load_from_file(MODELPATH % self.rfinder.settings.MODEL_TYPE)
    ...
    def save_instance(self, outfile, protocol=-1):
        ...
        rea_aa_dict = dict((aa_letters_1to3[k], map(lambda vv:aa_letters_1to3[vv], v.keys())) for k,v in self.rfinder.aa2aa_rea.items())        
        with open(outfile+".aa", 'w') as AA:
            yaml.dump(rea_aa_dict, AA, default_flow_style=False, Dumper=AADumper)
