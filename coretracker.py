...
if __name__ == '__main__':
    ...
    if(args.excludegap):
        ...
        length = alignment.get_alignment_length()
        alignment_save = alignment
        print "Alignment length after removing gaps" , length
...
#~ 
    ...
    if IC_INFO_THRESHOLD:
        ...
        length = alignment.get_alignment_length()
        print "Alignment length after removing low IC columns ",  length


    # Filter using the match percent per columns
    # Already enabled by default in the arguments list to filter 
    if(args.idfilter):
        filtered_alignment = filter_alignment(filtered_alignment, threshold=(abs(args.idfilter) <= 1 or 0.01) * abs(args.idfilter))
        AlignIO.write(filtered_alignment, open(MAFFT_OUTPUT + "_filtered", 'w'), 'fasta')
        alignment = filtered_alignment
        length = alignment.get_alignment_length()
        print "Alignment length after removing columns less conserved than threshold ", length

        
        # no keeping a variable for this
        filtered_alignment = filter_alignment(filtered_alignment, remove_identity=True, threshold=(abs(args.idfilter) <= 1 or 0.01) * abs(args.idfilter))
        AlignIO.write(filtered_alignment, open(MAFFT_OUTPUT + "_matchremoved", 'w'), 'fasta')
        alignment = filtered_alignment
        length = alignment.get_alignment_length()
        print "Alignment length after removing columns less conserved than threshold and 100% identical " , length
    ...
    for aa in aa_letters:
        ...
        if(cons_array):
            ...
            AlignIO.write(aa_filtered_alignment, open(
            TMP + "alignement_" + aa + ".ali", 'w'), 'fasta')
            alignment = aa_filtered_alignment
            length = alignment.get_alignment_length()
            print "Alignment length for ", aa , " columns" , length
